...
export function resolveResult(a: Answers): ResultKey {
    ...
    if (a[4] === "a") {
          if (a[3] === "c") return "METODO";
          if (a[3] === "b") return "METODOPRECIFICACAO";
          return "PLANILHA";
    }
    if (a[4] === "b") {
...
export function getResultContent(key: ResultKey): ResultContent {
    switch (key) {
      ...
      case "METODOPRECIFICACAO":
              return {
                        badge: "Seu diagnóstico: já tentou sozinho, não virou sistema",
                        urgency: { text: "Curso completo, não só ferramenta", tone: "soft" },
                        headline: {
                                    struck: "Mais um vídeo não resolve isso.",
                                    pre: "Falta",
                                    accent: "método",
                                    post: ", não dica solta.",
                        },
                        compare: {
                                    wrong: "um vídeo ou dica resolvia",
                                    right: "faltava aula estruturada com quem já precificou 4 lojas",
                        },
                        closing: "15 aulas com o Yago e o Ítalo, não mais um vídeo solto.",
                        proofCaption: "Aulas direto com o Yago e o Ítalo",
                        anticipation: "Libere abaixo o acesso ao Método de Precificação.",
                        ctas: [
                          {
                                        label: "Quero o Método de Precificação",
                                        kind: "link",
                                        href: withOrigem(LINK_METODO_PRECIFICACAO, "raiox_metodoprecificacao"),
                                        variant: "primary",
                          },
                                  ],
              };
...
export function buildChips(key: ResultKey, a: Answers): string[] {
  ...
  if (chips.length < 3) {
        ...
        if (key === "METODOPRECIFICACAO") chips.push("Curso, não planilha");
